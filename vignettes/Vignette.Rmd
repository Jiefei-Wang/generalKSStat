...
Though the idea of the KS statistic is straightforward, one may argue that the statistic cannot provide a proper power if the true distribution only deviates from the null distribution at two tails, that is, the KS test has a weak power to detect the difference between $\hat{F}(x)$ and $F(x)$ for $x$ closed to 0 or 1. The reason is simple, the variances of $\hat{F}(x)$ are not the same for all $x$, the maximum occurs at $x=0.5$. Therefore, the distance $\left|\hat{F}(x)-F(x)\right|$ is usually large for $x$ in the middle even when the null hypothesis is true, this property inflates the rejection region of the KS statistic and makes it hard to detect the difference at tails. 

Suppose that $X_1,X_2,...,X_n$ are n iid samples from the true distribution. Let $X_{(1)},X_{(2)},...,X_{(n)}$ be the ordered statistics of the samples where $X_{(1)}\leq X_{(2)}\leq ...\leq X_{(n)}$, The KS statistic can be written as
...
The original higher criticism(HC) statistic is proposed for dealing with pvalues and thus only has an one-sided formula, here we use the same idea and generalize to make it consistent with the KS statistic.
...
Though the weights are different for the aforementioned statistics, they share a common formula. The rejection rule of these statistics can be characterized by two sequences $\{l_u\}$ and $\{u_i\}$, that is
...
where the sequences $\{l_i\}$ and $\{u_i\}$ is determined by the statistics. remarkably, the one-sided version of the aforementioned statistics(e.g. $KS^+$) also fits into the above rejection rule where $\{l_i\}$ or $\{u_i\}$ euqals 0 depending on the side. Therefore, by controlling the value of $\{l_i\}$ and $\{u_i\}$, we can control which area of $F(x)$ that will be detected. In the package, this can be done via the parameter $alpha0$, $index$,$indexU$,$indexL$.
...
## One-sided KS+ stat
GKSStat(x = x, indexL = indexL, statName = "KS")

## One-sided HC+ stat
GKSStat(x = x, indexL = indexL, statName = "HC")

## One-sided BJ+ stat
GKSStat(x = x, indexL = indexL, statName = "BJ")
```
The parameter `indexL` corresponds to the detection range of the tests that favor right-skewed distribution(e.g. $KS^+$, $BJ^+$ and $HC^+$), and `indexU` corresponds to the left-skewed tests(e.g. $KS^-$, $BJ^-$ and $HC^-$). A combination of `indexL` and `indexU` is supported but the use of it should be justified by the alternative distribution. An equivalent form of the above tests are
```{r}
index <- 25L : 50L
...
GKSStat(x = x, index = index, statName = "BJ+")
```
These will yield the same results as the previous code chunk shows.
